...
async function getOrderFlow(symbol, depthLimit = 50, tradesLimit = 200) {
  const [depthRes, tradesRes] = await Promise.all([
    fetchWithTimeout(`${BASE}/api/v3/depth?symbol=${encodeURIComponent(symbol)}&limit=${depthLimit}`),
    fetchWithTimeout(`${BASE}/api/v3/aggTrades?symbol=${encodeURIComponent(symbol)}&limit=${tradesLimit}`),
  ]);
