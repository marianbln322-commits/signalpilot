...
// Ties are refunds — void, not losses. Excluded from the denominator.
function agg(arr) {
  const graded = arr.filter((e) => typeof e.win === 'boolean');
  const n = graded.length;
  const wins = graded.filter((e) => e.win).length;
  return { n, wins, ties: arr.length - n, winRate: n ? +((wins / n) * 100).toFixed(1) : null };
}
