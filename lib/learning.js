...
// Detailed bootstrap progress. Interim win-rates are display-only and never
// unlock execution until the exact cohort reaches the configured minimum.
function calibrationProgress(entries, symbols = [], minSample = DEFAULT_MIN_SAMPLE) {
  const keys = [];
  for (const symbol of symbols) {
    for (const interval of ['10 minute', '30 minute']) {
      for (const directie of ['UP', 'DOWN']) keys.push(`${symbol}-${directie}-${interval}`);
    }
  }
  for (const entry of entries) {
    const key = `${entry.symbol}-${entry.directie}-${entry.interval}`;
    if (!keys.includes(key)) keys.push(key);
  }

  return keys.map((key) => {
    const matching = entries.filter((entry) => `${entry.symbol}-${entry.directie}-${entry.interval}` === key);
    const resolved = matching.filter((entry) => entry.status === 'resolved' && entry.settlementSource === 'aggTrade-exact');
    const stats = agg(resolved);
    const lastSeparator = key.lastIndexOf('-');
    const interval = key.slice(lastSeparator + 1);
    const symbolDirection = key.slice(0, lastSeparator);
    const directionSeparator = symbolDirection.lastIndexOf('-');
    const symbol = symbolDirection.slice(0, directionSeparator);
    const directie = symbolDirection.slice(directionSeparator + 1);
    return {
      key,
      symbol,
      directie,
      ...
      pending: matching.filter((entry) => entry.status === 'pending').length,
      void: matching.filter((entry) => entry.status === 'void').length,
      required: minSample,
      remaining: Math.max(0, minSample - stats.n),
      ready: stats.n >= minSample,
    };
  });
}
...
module.exports = { analyze, evaluate, summary, calibrationProgress, wilsonLowerBound, DEFAULT_MIN_SAMPLE };
