...
function evaluate(entries, ctx, minSample = DEFAULT_MIN_SAMPLE) {
  ...
  if (ctx.interval) pull(a.byInterval, ctx.interval, `fereastră ${ctx.interval}`);
  if (ctx.ofAgree) pull(a.byOfAgree, `of:${ctx.ofAgree}`, `order flow ${ctx.ofAgree}`);

  // Forecast calibration is horizon-specific. Broad marginal buckets (hour,
  // symbol/direction, setup) may refine an estimate but cannot unlock a 10m or
  // 30m forecast without enough outcomes from that exact horizon.
  if (ctx.requireInterval) {
    const intervalBucket = a.byInterval[ctx.interval];
    if (!intervalBucket || intervalBucket.n < minSample || intervalBucket.winRate == null) {
      const n = intervalBucket ? intervalBucket.n : 0;
      return {
        ready: false,
        estimate: null,
        adjustment: 0,
        factors,
        note: `calibrare ${ctx.interval}: ${n}/${minSample} rezultate forward exacte`,
      };
    }
  }
  ...
  if (ctx.requireInterval) {
    const intervalBucket = a.byInterval[ctx.interval];
    const estimate = intervalBucket.winRate;
    return {
      ready: true,
      estimate,
      adjustment: +(estimate - 50).toFixed(1),
      factors,
      note: `probabilitate empirică ${ctx.interval}: ${estimate}% din ${intervalBucket.n} rezultate forward exacte`,
    };
  }
