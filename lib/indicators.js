...
// Session-anchored VWAP. Resets at `sessionHourUTC` every UTC day instead of
// silently changing its economic meaning between 5m and 15m timeframes.
function sessionVwap(candles, sessionHourUTC = 0) {
  const out = new Array(candles.length).fill(null);
  let sessionKey = null;
  let pv = 0;
  let vol = 0;
  for (let i = 0; i < candles.length; i++) {
    const ts = candles[i].openTime != null ? candles[i].openTime : i;
    const shifted = new Date(ts - sessionHourUTC * 60 * 60 * 1000);
    const key = `${shifted.getUTCFullYear()}-${shifted.getUTCMonth()}-${shifted.getUTCDate()}`;
    if (key !== sessionKey) {
      sessionKey = key;
      pv = 0;
      vol = 0;
    ...
    const barVolume = Number(candles[i].volume) || 0;
    pv += tp * barVolume;
    vol += barVolume;
    out[i] = vol > 0 ? pv / vol : null;
  }
