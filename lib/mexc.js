...
// Resolve a recent event from the closed 1m candle containing the requested
// expiry. This is deterministic and survives resolver delays; callers should
// still configure the exact settlement source required by their contract.
async function fetchSettlementPrice(symbol, expiryTs) {
  const parse = (raw) => (Array.isArray(raw) ? raw : []).map((r) => ({
    openTime: Number(r[0]), open: Number(r[1]), high: Number(r[2]), low: Number(r[3]),
    close: Number(r[4]), volume: Number(r[5]), closeTime: Number(r[6]), quoteVolume: Number(r[7]),
  }));
  const endTime = Math.floor(Number(expiryTs) + 60 * 1000);
  const url = `${BASE}/api/v3/klines?symbol=${encodeURIComponent(symbol)}&interval=1m&limit=10&endTime=${endTime}`;
  const res = await fetch(url, { headers: { Accept: 'application/json' } });
  const around = res.ok ? parse(await res.json()) : [];
  const candles = around.length ? around : await fetchKlines(symbol, '1m', 200);
  const target = candles.find((c) => c.openTime <= expiryTs && c.closeTime >= expiryTs - 1);
  if (target) {
    if (target.closeTime >= Date.now()) throw new Error(`Settlement candle for ${symbol} is still open`);
    return { price: target.close, source: 'MEXC spot 1m close proxy', candle: target };
  }
  throw new Error(`Settlement candle unavailable for ${symbol} at ${expiryTs}`);
}
