...
În warm-up learnerul monitorizează și nu blochează nimic. Un bucket devine `ACTIVE` numai după minimum **160** exemple efective, minimum **25 WIN** și **25 LOSS**, iar log-loss-ul prequential al modelului trebuie să fie mai bun decât baseline-ul naiv calculat numai din trecut. După activare, pragul fix este `1 / (1 + payout) + 3 puncte procentuale`. Learnerul poate numai să păstreze un `ENTER` determinist sau să îl transforme în `WAIT`; nu poate crea o intrare, inversa `UP/DOWN`, aproba automat o tranzacție sau executa ordine. Progresul, sample count-ul, overlap-urile excluse și comparația model/baseline sunt afișate separat pentru fiecare bucket.
