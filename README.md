...
Interfața arată un singur grafic per simbol, implicit **15m**, cu butoane **1m / 5m / 15m / 30m / 60m**. Deasupra graficului, panoul multi-timeframe afișează pentru fiecare interval direcția EMA stack, starea `ESTABLISHED/DEVELOPING/CHOPPY/NEUTRAL`, persistența, efficiency ratio și panta EMA20 normalizată la ATR. Graficul păstrează EMA9/20/50, MAX50/MIN50 și balanța relativă UP/DOWN; această balanță este un scor al dovezilor, nu probabilitate de WIN.

Motorul 10m cere acord dominant și trend persistent obligatoriu pe **1m + 5m**, confirmare primară pe **15m**, iar **30m + 60m** pot aplica macro-veto. Motorul 30m cere acord și trend persistent pe **5m + 15m**, cu **30m + 60m** pentru confirmarea regimului și veto. Trendul nu este dedus doar din ultima separare EMA: combină EMA stack, pantă EMA20/ATR, persistență, efficiency ratio, consistența close-urilor, structură, momentum, volum și volatilitate. Triggerul trebuie să fie proaspăt, suficient de puternic și neinvalidat ulterior la close. Un timeframe de context nu poate înlocui unul dintre cele două timeframe-uri obligatorii, iar intrările extinse la peste 2,2 ATR de EMA20 sunt blocate fail-closed.
...
Protecția locală nouă, activă **numai în instanța 3014**, are patru trepte. Instanța 3013 folosește aceeași bază de cod și aceeași interfață statică, dar profilul ei fix dezactivează lecțiile contextuale, circuitul rapid și setup guard-ul și păstrează learnerul logistic anterior; datele celor două instanțe rămân separate. Prima treaptă funcționează imediat: după primul LOSS pe care aplicația l-a permis, salvează un diagnostic cu **cauze candidate** și o soluție restrictivă pentru exact `contextFingerprint`-ul tehnic (roluri MTF, direcție/stare trend, persistență, efficiency, structură/momentum, tip/prospețime trigger și extensie). Următorul ENTER în același context devine WAIT și continuă să fie urmărit shadow. Un WIN shadow eligibil, non-overlapping, reabilitează contextul; un LOSS shadow îl menține blocat și actualizează diagnosticul. Lecția nu schimbă global strategia și nu pretinde că demonstrează cauza mișcării.

A doua treaptă 3014 funcționează separat, inclusiv în warm-up: după **3 rezultate LOSS consecutive** dintre semnalele pe care aplicația le-ar fi permis într-un bucket, circuitul acelui bucket se deschide și toate noile ENTER devin WAIT. Candidaturile continuă să fie rezolvate în shadow; circuitul se închide numai când ultimele **5 observații shadow eligibile, non-overlapping**, conțin suficiente WIN pentru pragul `break-even + 3pp`. A treia treaptă urmărește separat fiecare `setupFingerprint`: după 12 rezultate efective, setup-ul este blocat dacă limita superioară Wilson95 rămâne sub același prag și este reabilitat automat numai când noile rezultate shadow îi refac statistica.

A patra treaptă este modelul logistic complet. Un bucket devine `ACTIVE` numai după minimum **160** exemple efective, minimum **25 WIN** și **25 LOSS**, iar log-loss-ul prequential al modelului trebuie să fie mai bun decât baseline-ul naiv calculat numai din trecut. După activare, modelul blochează probabilitățile sub `1 / (1 + payout) + 3 puncte procentuale`. Toate cele patru trepte pot numai să păstreze un `ENTER` determinist sau să îl transforme în `WAIT`; nu pot crea o intrare, inversa `UP/DOWN`, aproba automat o tranzacție sau executa ordine. Progresul, lecțiile contextuale și soluțiile lor, circuitul, recovery shadow, setup-urile slabe, overlap-urile excluse și comparația model/baseline sunt afișate separat pentru fiecare bucket.
