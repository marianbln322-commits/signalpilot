...
function multiTimeframeTrendPanel(result) {
  const prediction = result && result.predictions && (result.predictions['10m'] || result.predictions['30m']);
  const trend = prediction && prediction.multiTimeframeTrend;
  const frames = trend && Array.isArray(trend.frames) ? trend.frames : [];
  if (!frames.length) return '<div class="mtf-trend"><span class="muted">Aștept profilul de trend 1m–60m…</span></div>';
  const chips = frames.map((frame) => {
    const visual = ['ESTABLISHED', 'DEVELOPING'].includes(frame.state) ? directionClass(frame.direction) : 'NEUTRAL';
    return `<div class="mtf-chip ${safe(visual)}"><b>${safe(frame.timeframe)} · ${safe(frame.direction)}</b><span>${safe(frame.state)} · P ${safe(formatNumber(Number(frame.persistence) * 100, 0))}% · E ${safe(formatNumber(Number(frame.efficiency) * 100, 0))}% · slope ${safe(formatNumber(frame.ema20SlopeAtr, 2))} ATR</span></div>`;
  }).join('');
  return `<div class="mtf-trend"><div class="mtf-title"><b>Citire constantă multi-timeframe</b><span>consens established: ${safe(trend.consensus)} · UP ${safe(trend.establishedUp)} / DOWN ${safe(trend.establishedDown)}</span></div><div class="mtf-grid">${chips}</div></div>`;
}
...
function marketChart(symbol, result, timeframe, ticker) {
  ...
  const profile = analysis && analysis.trendProfile || {};
  const badges = analysis ? `<div class="analysis-strip"><span class="${safe(directionClass(analysis.trend))}">trend ${safe(analysis.trend)} · ${safe(profile.state || '—')}</span><span>P ${safe(formatNumber(Number(profile.persistence) * 100, 0))}%</span><span>E ${safe(formatNumber(Number(profile.efficiency) * 100, 0))}%</span><span>slope ${safe(formatNumber(profile.ema20SlopeAtr, 2))} ATR</span><span class="${safe(directionClass(analysis.momentum))}">momentum ${safe(analysis.momentum)}</span><span class="${safe(directionClass(analysis.structure))}">structură ${safe(analysis.structure)}</span><span>regim ${safe(analysis.regime)}</span><span>RSI ${safe(formatNumber(analysis.rsi))}</span><span>ATR ${safe(formatNumber(analysis.atrPct, 3))}%</span><span>volum ${safe(formatNumber(analysis.volumeRatio))}x</span></div>` : '';
...
function localLearningBlock(learning) {
  ...
  const protection = learning.adaptiveProtection !== true
    ? 'instanța 3013: protecția adaptivă este dezactivată; folosește 3014 pentru cele 4 trepte'
    : circuit.open
  ...
  const lesson = learning.contextLesson || {};
  const lessonText = lesson.active
    ? `<div class="context-lesson"><b>Lecție context activă:</b> ${safe((lesson.causeCodes || []).join(', ') || 'fără cauză unică')}<br><b>Soluție restrictivă:</b> ${safe(lesson.solutionDetail || lesson.solutionCode || 'confirmare nouă')}<br><b>Shadow:</b> ${safe(lesson.shadowWins || 0)}W/${safe(lesson.shadowLosses || 0)}L din ${safe(lesson.shadowEvaluated || 0)}</div>`
    : lesson.recoveredAt
      ? `<div class="context-lesson recovered"><b>Lecție context reabilitată:</b> WIN shadow eligibil la ${safe(formatTime(lesson.recoveredAt))}</div>`
      : '';
  const reasons = (learning.blockReasons || []).map((reason) => `${reason.code}: ${reason.detail}`);
  return `<div class="learner-box ${safe(stateClass)}"><div class="learner-head"><b>Learner local · ${safe(phase)}</b><span>${safe(learning.blocked ? 'ENTER BLOCAT' : learning.active ? 'ENTER PĂSTRAT' : learning.adaptiveProtection ? 'PROTECȚIE ADAPTIVĂ ACTIVĂ' : 'LEARNER 3013')}</span></div><div>${safe(learning.bucketKey)} · N efectiv ${safe(learning.effectiveSamples)}/${safe(learning.minimumEffectiveSamples)} · W/L ${safe(learning.wins)}/${safe(learning.losses)}${safe(probability)}</div><div><b>Protecție:</b> ${safe(protection)}</div>${lessonText}${reasons.length ? `<div><b>Ce a corectat:</b>${compactList(reasons)}</div>` : ''}<p>${safe(eligibility)} · learnerul poate doar bloca, niciodată crea sau inversa semnale.</p></div>`;
}
...
function decisionCard(symbol, prediction) {
  ...
  const reasons = (prediction.reasonCodes || []).slice(0, 7).map((reason) => `<span class="reason">${safe(reason)}</span>`).join('');
  const executionRoles = new Set(prediction.executionTimeframes || []);
  const primaryRoles = new Set(prediction.primaryContextTimeframes || []);
  const macroRoles = new Set(prediction.macroContextTimeframes || []);
  const trendFrames = prediction.multiTimeframeTrend && prediction.multiTimeframeTrend.frames || [];
  const roleFor = (timeframe) => [executionRoles.has(timeframe) ? 'exec' : null, primaryRoles.has(timeframe) ? 'primary' : null, macroRoles.has(timeframe) ? 'macro' : null].filter(Boolean).join('+') || 'context';
  const frameRows = trendFrames.map((frame) => `<span class="frame-chip ${safe(directionClass(['ESTABLISHED', 'DEVELOPING'].includes(frame.state) ? frame.direction : 'NEUTRAL'))}"><b>${safe(frame.timeframe)} ${safe(roleFor(frame.timeframe))}</b> · ${safe(frame.direction)}/${safe(frame.state)} · P${safe(formatNumber(Number(frame.persistence) * 100, 0))}% E${safe(formatNumber(Number(frame.efficiency) * 100, 0))}% S${safe(formatNumber(frame.ema20SlopeAtr, 2))}</span>`).join('');
  const learning = prediction.localLearning;
  const balance = analysisBalance(prediction);
  const mtf = prediction.multiTimeframeTrend || {};
  ...
  return `<article class="decision-card ${safe(visual)}"><div class="decision-top"><div><span class="symbol-label">${safe(symbol)}</span><h3>${safe(prediction.horizonMin)} minute</h3></div><span class="quality-ring">${safe(prediction.quality)}<small>/100</small></span></div><div class="decision-verdict ${safe(visual)}">${safe(title)}</div><p class="protocol">${safe(prediction.protocol)}</p><div class="frame-row">${frameRows}</div><div class="reason-codes">${reasons}</div><div class="decision-facts"><span>Bias <b class="${safe(directionClass(prediction.bias))}">${safe(prediction.bias)}</b> · consens MTF established <b class="${safe(directionClass(mtf.consensus))}">${safe(mtf.consensus || 'NEUTRAL')}</b></span><span>${safe(balanceText)} · scor relativ, nu probabilitate WIN</span><span>Trigger: ${safe(trigger)}</span><span>${safe(probabilityText(prediction))}</span></div>${localLearningBlock(learning)}<details class="audit"><summary>Audit complet al deciziei</summary><div class="audit-grid"><b>Execuție</b><span>${safe((prediction.executionTimeframes || []).join(' + '))}</span><b>Context primar</b><span>${safe((prediction.primaryContextTimeframes || []).join(' + ') || '—')}</span><b>Macro / veto</b><span>${safe((prediction.macroContextTimeframes || []).join(' + ') || '—')}</span><b>Consens MTF</b><span>${safe(mtf.consensus || 'NEUTRAL')} · established UP ${safe(mtf.establishedUp || 0)} / DOWN ${safe(mtf.establishedDown || 0)}</span><b>Confirmări</b><span>${safe((prediction.confirmations || []).join(', ') || '—')}</span><b>Dovezi</b><span>${compactList(prediction.evidence)}</span><b>Conflicte</b><span>${compactList(prediction.conflicts, 'niciun conflict explicit')}</span><b>Trigger invalidat</b><span>${safe((prediction.invalidatedTriggerCandidates || []).length)} candidat(e) în fereastră</span><b>Invalidare</b><span>${safe(prediction.invalidation)}</span><b>Entry</b><span>${safe(formatTime(prediction.entryBoundaryOpenTime))}</span><b>Expiry</b><span>${safe(formatTime(prediction.expiryEstimateCloseTime))}</span><b>Porți</b><span>${gatesTable(prediction.gateChecks)}</span><b>Context key</b><span class="fingerprint">${safe(prediction.contextFingerprint || '—')}</span><b>Signal key</b><span class="fingerprint">${safe(prediction.signalKey || '—')}</span></div></details></article>`;
}
...
function renderLocalLearning(learner) {
  ...
  status.textContent = unavailable
    ...
    : learner && learner.externalApi === false
      ? adaptiveProtection
        ? `${globalPhase} · cost API 0 · N efectiv ${learner.totals && learner.totals.effectiveSamples || 0} · lecții active ${learner.totals && learner.totals.activeContextLessons || 0} / contexte învățate ${learner.totals && learner.totals.learnedContexts || 0} · circuit după ${learner.activation && learner.activation.lossStreakLimit || 3} loss`
  ...
  byId('localLearningSummary').innerHTML = buckets.length ? buckets.map((bucket) => {
    ...
    const lessons = bucket.activeContextLessons || [];
    const lessonDetails = lessons.length ? `<div class="bucket-lessons"><b>Lecții context active (${safe(lessons.length)}):</b>${compactList(lessons.map((lesson) => `${(lesson.causeCodes || []).join(', ') || 'fără cauză unică'} → ${lesson.solutionCode || 'confirmare nouă'} · shadow ${lesson.shadowWins || 0}W/${lesson.shadowLosses || 0}L · blocări ${lesson.blockedCount || 0}`))}</div>` : '';
    return `<article class="learner-bucket ${safe(phaseClass)}"><div><b>${safe(bucket.symbol)} · ${safe(bucket.horizonMin)}m</b><span>${safe(circuit.open ? 'PAUZĂ ADAPTIVĂ' : bucket.phase)}</span></div><strong>N ${safe(bucket.effectiveSamples)}/${safe(bucket.minimumEffectiveSamples)}</strong><p>W/L ${safe(bucket.wins)}/${safe(bucket.losses)} · pending ${safe(bucket.pending)} · overlap exclus ${safe(bucket.excludedOverlap)} · invalid ${safe(bucket.invalid)}</p><p><b>Protecție:</b> ${protection}</p>${lessonDetails}<small>${safe(losses)}</small></article>`;
